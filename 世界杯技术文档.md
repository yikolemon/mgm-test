...
* 每日清零采用“两步 SQL”方案：发放前/扣减前先执行条件清零，再执行要求 `updatetime` 已在今天的发放/扣减 SQL → 不再使用 `expire_time + Job`
...
sequenceDiagram
  ...
  alt 命中幂等或频控
    ...
    Core->>DB: 条件清零 activity_account（仅当 updatetime < todayStart）
    Core->>DB: 写入 activity_task_record(awardFlag=true, bizNo)
    Core->>DB: 更新 activity_account（要求 updatetime >= todayStart，直接 total_value + awardValue、available_value + awardValue）
    Core-->>Caller: 返回最新剩余次数
  end

  Caller->>Core: drawLottery(activityCode, userId)
  Core->>DB: 条件清零 activity_account（仅当 updatetime < todayStart）
  Core->>DB: 更新 activity_account（要求 updatetime >= todayStart 且 available_value > 0，再执行 available_value - 1）
  Core->>DB: 先完成抽奖结果计算
  alt 中奖
    ...
    alt 总库存已耗尽
      ...
      Note over Core,Redis: 若占用失败，需按超发保护降级为空奖
      Core->>DB: 原子扣减总库存（`granted_total = granted_total + 1` 且 `granted_total < total_stock`）
      Note over Core,DB: 若总库存扣减失败，需按超发保护降级为空奖
...
flowchart TD
  ...
  N -- 是 --> P[先抽奖]
  P --> Q{命中奖项总库存是否已耗尽}
  Q -- 是 --> R[降级为空奖并落最终未中奖记录]
  Q -- 否 --> S[尝试占用 Redis 日库存]
  S --> T{Redis 占用成功?}
  T -- 否 --> R
  T -- 是 --> U[原子扣减总库存]
  U --> V{DB 扣减成功?}
...
CREATE TABLE `activity_account` (
  ...
  `isactive` tinyint(1) NOT NULL DEFAULT '1' COMMENT '逻辑删除',
  PRIMARY KEY (`id`),
  KEY `idx_user_activity_account` (`user_id`, `activity_id`, `account_type`),
...
CREATE TABLE `activity_reward_record` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `activity_id` bigint(20) NOT NULL DEFAULT 0 COMMENT '活动ID',
  `lottery_record_id` bigint(20) NOT NULL DEFAULT 0 COMMENT '抽奖记录ID',
  `user_id` bigint(20) NOT NULL DEFAULT 0 COMMENT '用户ID',
  `reward_config_id` bigint(20) NOT NULL DEFAULT 0 COMMENT '奖励配置ID',
...
* 发放前先执行一次条件清零 SQL：仅当 `activity_account.updatetime` 不属于当前墨西哥自然日时，将 `available_value` 置 0 并更新时间
* 发放 SQL 只在 `activity_account.updatetime` 已属于当前墨西哥自然日时执行，并直接基于数据库当前值完成 `+awardValue`
...
* 查询前若发现 `activity_account.updatetime` 已跨自然日，则先执行一次条件清零，再返回结果
...
* 抽奖前先执行一次条件清零 SQL；随后扣减 SQL 仅在 `activity_account.updatetime` 已属于当前自然日且 `available_value > 0` 时执行
* 若扣减 SQL 更新影响行数为 0，则视为剩余次数不足
* 先按权重抽出命中奖项，再检查该奖项总库存是否已耗尽；若已耗尽，则将本次结果降级为空奖

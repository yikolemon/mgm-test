...
4. 命中新流程的授信申请会在 `loan-biz-service` 的授信快照中写入 `new_user_info_flow` 标识，供下游策略判断是否绕过 UBT 模型。
...
sequenceDiagram
  ...
  AppServer ->> FlowEngine: POST /flow/v1/latest-record(userId)
  FlowEngine ->> FlowDB: 按 userId 查询最近一条填资流程记录
  ...
  alt latest-record 查询失败
    ...
    AppServer ->> CapsuleBox: POST /capsule-box/abtest/getLatestHistoryGroupResult
    alt 历史灰度命中 0/1
      AppServer ->> AppServer: 历史 groupName 映射 flowKey
    else 历史未命中
      AppServer ->> CapsuleBox: POST /capsule-box/abtest/getGroupResult(saveResultRecord=true)
      CapsuleBox -->> AppServer: 当前 groupName
    end
    AppServer ->> FlowEngine: POST /flow/v1/current-step(userId, selectedFlowKey)
  end
...
```mermaid
flowchart TD
  A[App 提交用户资料] --> B[app-server 查询 latest-record]
  B --> C{latest-record 是否成功?}
  C -- 否 --> X[返回系统错误, 不推进流程]
  C -- 是 --> D{是否存在未完成流程?}
  D -- 是 --> E[使用未完成流程 flowKey]
  D -- 否 --> F[按历史灰度/当前灰度选择 flowKey]
...
sequenceDiagram
  ...
  LoanBiz ->> FlowEngine: POST /flow/v1/latest-completed-record(userId)
  FlowEngine -->> LoanBiz: 最近一次已完成填资流程 flowKey
  LoanBiz ->> Mongo: 保存 credit_apply_detail_snapshot.new_user_info_flow
...
| --- | --- | --- | --- | --- | --- |
| 新增 | `new_user_info_flow` | Boolean | YES | `false` | 本次授信是否使用 `userInfo-optimized-flow` |

说明：MongoDB 文档新增字段无需 DDL；历史快照不回填，读取方按 `null=false` 处理。
...
内部接口，仅供 `loan-biz-service` 创建授信快照时查询用户最近一次已完成的填资流程。授信快照字段 `new_user_info_flow` 由 flow-engine 返回的已完成流程 `flowKey` 决定：最近一次已完成流程为 `userInfo-optimized-flow` 时写入 `true`，否则写入 `false`。
...
| ├── newUserInfoFlow | boolean | 最近一次已完成流程是否为 `userInfo-optimized-flow` |
...
```json
{
  ...
  "data": {
    "exists": true,
    "flowKey": "userInfo-optimized-flow",
    "newUserInfoFlow": true
  }
}
```

说明：该接口只查询已完成流程记录，不调用当前灰度、不启动流程、不读取未完成流程。无已完成流程、查询失败或返回非 `userInfo-optimized-flow` 时，loan-biz-service 写入 `new_user_info_flow=false` 并记录 error，保证授信申请不中断。
...
| 站点 | key | value | 说明 |
| --- | --- | --- | --- |
| app-server.creocreditapi.mx | `userInfo.flow` | `userInfo-default-flow` | 保留，老流程默认 flowKey |
| app-server.creocreditapi.mx | `userInfo.optimizedFlow` | `userInfo-optimized-flow` | 新增，新流程 flowKey |
| app-server.creocreditapi.mx | `userInfo.flowKeys` | `userInfo-default-flow,userInfo-optimized-flow` | 新增，未完成流程互斥检查范围 |
| app-server.creocreditapi.mx | `userInfo.gray.creditFlowOptimization.code` | `creditFlowOptimization` | 新增，capsule-box 实验 code |
| app-server.creocreditapi.mx | `userInfo.gray.creditFlowOptimization.defaultGroup` | `0` | 新增，灰度异常默认老流程 |
| flow-engine.creocreditapi.mx | `step.basic-info-optimized.schema` | `<schema-json>` | 新增，新流程基础信息/职业信息合并页 schema |
| loan-biz-service.creocreditapi.mx | `user.slot.config` | 增加 `node=/applying/?step=basic-info-optimized#back` 的 `creditRetentionTip` 配置 | 新增，兼容新流程戳额挽留 |
| capsule-box.creocreditapi.mx | `creditFlowOptimization` | `0=100%,1=0%` | 新增实验配置，上线默认不放量新流程 |
...
5. loan-biz-service 授信快照写入逻辑，新增 `new_user_info_flow` 标识。
...
8. loan-biz-service 授信快照 `new_user_info_flow` 写入缺失率；B 组用户缺失率超过 1% 需告警。
...
说明：虽然本需求会影响授信前用户填资链路，并在授信快照中新增标识字段，但上线默认新流程 `1=0%`，且打标异常降级为 `false` 不阻断授信申请；整体通过灰度和 fail-closed 策略控制风险。
